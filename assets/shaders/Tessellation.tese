...
void main()
{
	...
	float d = max(textureLod(TextureSamplers[texId], outUV.st, 0.0).r, 0.0);
	d = clamp(d, -0.5, 0.5);

	gl_Position.xyz += normalize(outNormal) * (d * tessStrength);
		
	vec3 light = vec3(5, 10, 30);
	outLight = normalize(normalize(light) - gl_Position.xyz);
