...
Icosphere::Icosphere(const float radius, const int subdivision, const bool smooth) :
	radius_(radius), subdivision_(subdivision), smooth_(smooth)
{
    if(smooth)
        buildVerticesSmooth();
    else
...
///////////////////////////////////////////////////////////////////////////////
std::vector<float> Icosphere::computeIcosahedronVertices() const
{
    ...
    {
        i1 = i * 3;         // for 2nd row
        const int i2 = (i + 5) * 3;   // for 3rd row

        const float z = radius_ * sin(V_ANGLE);              // elevaton
        const float xy = radius_ * cos(V_ANGLE);
...
void Icosphere::buildVerticesFlat()
{
    ...
    for(int i = 1; i <= 5; ++i)
    {
        ...
        const float* v1 = &tmpVertices[i * 3];
        const float* v2 = &tmpVertices[(i < 5 ? (i + 1) * 3 : 3)];
        const float* v3 = &tmpVertices[(i + 5) * 3];
        const float* v4 = &tmpVertices[(i + 5 < 10 ? i + 6 : 6) * 3];
        ...
        //   \   \   \   \   \      : (i+9,i+10), (i+9, i+11)               //
        //    \   \   \   \   \                                             //
        lineIndices_.push_back(index);       // (i, i+1)
        lineIndices_.push_back(index+1);       // (i, i+1)
        lineIndices_.push_back(index+3);     // (i+3, i+4)
        lineIndices_.push_back(index+4);
        lineIndices_.push_back(index+3);     // (i+3, i+5)
        lineIndices_.push_back(index+5);
        lineIndices_.push_back(index+4);     // (i+4, i+5)
        lineIndices_.push_back(index+5);
        lineIndices_.push_back(index+9);     // (i+9, i+10)
        lineIndices_.push_back(index+10);
        lineIndices_.push_back(index+9);     // (i+9, i+11)
        lineIndices_.push_back(index+11);
...
void Icosphere::buildVerticesSmooth()
{
    ...
    std::vector<unsigned int>().swap(indices_);
    std::vector<unsigned int>().swap(lineIndices_);
    std::map<std::pair<float, float>, unsigned int>().swap(sharedIndices_);
    ...
    sharedIndices_[std::make_pair(S_STEP * 2, T_STEP)] = static_cast<unsigned>(texCoords_.size() / 2 - 1);
    ...
    sharedIndices_[std::make_pair(S_STEP * 4, T_STEP)] = static_cast<unsigned>(texCoords_.size() / 2 - 1);

    v[0] = tmpVertices[12]; v[1] = tmpVertices[13]; v[2] = tmpVertices[14]; // v16 (shared)
    const float scale = computeScaleForLength(v, 1);
    n[0] = v[0] * scale;    n[1] = v[1] * scale;    n[2] = v[2] * scale;
    addVertex(v[0], v[1], v[2]);
    addNormal(n[0], n[1], n[2]);
    addTexCoord(S_STEP * 6, T_STEP);
    sharedIndices_[std::make_pair(S_STEP * 6, T_STEP)] = static_cast<unsigned>(texCoords_.size() / 2 - 1);
    ...
    sharedIndices_[std::make_pair(S_STEP * 8, T_STEP)] = static_cast<unsigned>(texCoords_.size() / 2 - 1);
    ...
    sharedIndices_[std::make_pair(S_STEP * 3, T_STEP * 2)] = static_cast<unsigned>(texCoords_.size() / 2 - 1);
    ...
    sharedIndices_[std::make_pair(S_STEP * 5, T_STEP * 2)] = static_cast<unsigned>(texCoords_.size() / 2 - 1);
    ...
    sharedIndices_[std::make_pair(S_STEP * 7, T_STEP * 2)] = static_cast<unsigned>(texCoords_.size() / 2 - 1);
    ...
    sharedIndices_[std::make_pair(S_STEP * 9, T_STEP * 2)] = static_cast<unsigned>(texCoords_.size() / 2 - 1);
    ...
    lineIndices_.push_back(0);   lineIndices_.push_back(10);       // 00 - 10
    lineIndices_.push_back(1);   lineIndices_.push_back(14);       // 01 - 14
    lineIndices_.push_back(2);   lineIndices_.push_back(15);       // 02 - 15
    lineIndices_.push_back(3);   lineIndices_.push_back(16);       // 03 - 16
    lineIndices_.push_back(4);   lineIndices_.push_back(17);       // 04 - 17
    lineIndices_.push_back(10);  lineIndices_.push_back(14);       // 10 - 14
    lineIndices_.push_back(14);  lineIndices_.push_back(15);       // 14 - 15
    lineIndices_.push_back(15);  lineIndices_.push_back(16);       // 15 - 16
    lineIndices_.push_back(16);  lineIndices_.push_back(17);       // 10 - 14
    lineIndices_.push_back(17);  lineIndices_.push_back(11);       // 17 - 11
    lineIndices_.push_back(10);  lineIndices_.push_back(12);       // 10 - 12
    lineIndices_.push_back(12);  lineIndices_.push_back(14);       // 12 - 14
    lineIndices_.push_back(14);  lineIndices_.push_back(18);       // 14 - 18
    lineIndices_.push_back(18);  lineIndices_.push_back(15);       // 18 - 15
    lineIndices_.push_back(15);  lineIndices_.push_back(19);       // 15 - 19
    lineIndices_.push_back(19);  lineIndices_.push_back(16);       // 19 - 16
    lineIndices_.push_back(16);  lineIndices_.push_back(20);       // 16 - 20
    lineIndices_.push_back(20);  lineIndices_.push_back(17);       // 20 - 17
    lineIndices_.push_back(17);  lineIndices_.push_back(21);       // 17 - 21
    lineIndices_.push_back(21);  lineIndices_.push_back(11);       // 21 - 11
    lineIndices_.push_back(12);  lineIndices_.push_back(18);       // 12 - 18
    lineIndices_.push_back(18);  lineIndices_.push_back(19);       // 18 - 19
    lineIndices_.push_back(19);  lineIndices_.push_back(20);       // 19 - 20
    lineIndices_.push_back(20);  lineIndices_.push_back(21);       // 20 - 21
    lineIndices_.push_back(21);  lineIndices_.push_back(13);       // 21 - 13
    lineIndices_.push_back(5);   lineIndices_.push_back(12);       // 05 - 12
    lineIndices_.push_back(6);   lineIndices_.push_back(18);       // 06 - 18
    lineIndices_.push_back(7);   lineIndices_.push_back(19);       // 07 - 19
    lineIndices_.push_back(8);   lineIndices_.push_back(20);       // 08 - 20
    lineIndices_.push_back(9);   lineIndices_.push_back(21);       // 09 - 21
...
void Icosphere::subdivideVerticesFlat()
{
	...
	// iteration
    for(int i = 1; i <= subdivision_; ++i)
    {
        // copy prev arrays
        std::vector<float> tmpVertices = vertices_;
        std::vector<float> tmpTexCoords = texCoords_;
        std::vector<unsigned int> tmpIndices = indices_;
        ...
        unsigned int index = 0;
        const int indexCount = static_cast<int>(tmpIndices.size());

        for(int j = 0; j < indexCount; j += 3)
        {
            // get 3 vertice and texcoords of a triangle
            const float* v1 = &tmpVertices[tmpIndices[j] * 3];
            const float* v2 = &tmpVertices[tmpIndices[j + 1] * 3];
            const float* v3 = &tmpVertices[tmpIndices[j + 2] * 3];
            const float* t1 = &tmpTexCoords[tmpIndices[j] * 2];
            const float* t2 = &tmpTexCoords[tmpIndices[j + 1] * 2];
            const float* t3 = &tmpTexCoords[tmpIndices[j + 2] * 2];

            // get 3 new vertices by spliting half on each edge
            computeHalfVertex(v1, v2, radius_, newV1);
            computeHalfVertex(v2, v3, radius_, newV2);
            computeHalfVertex(v1, v3, radius_, newV3);
...
void Icosphere::subdivideVerticesSmooth()
{
    ...
    for(int i = 1; i <= subdivision_; ++i)
    {
        ...
        for(int j = 0; j < indexCount; j += 3)
        {
            ...
            const unsigned int i1 = tmpIndices[j];
            const unsigned int i2 = tmpIndices[j + 1];
            const unsigned int i3 = tmpIndices[j + 2];

            // get 3 vertex attribs from prev triangle
            const float* v1 = &vertices_[i1 * 3];
            const float* v2 = &vertices_[i2 * 3];
            const float* v3 = &vertices_[i3 * 3];
            const float* t1 = &texCoords_[i1 * 2];
            const float* t2 = &texCoords_[i2 * 2];
            const float* t3 = &texCoords_[i3 * 2];
            ...
            const unsigned int newI1 = addSubVertexAttribs(newV1, newN1, newT1);
            const unsigned int newI2 = addSubVertexAttribs(newV2, newN2, newT2);
            const unsigned int newI3 = addSubVertexAttribs(newV3, newN3, newT3);
...
{
    vertices_.push_back(v1[0]);  // x
    vertices_.push_back(v1[1]);  // y
    vertices_.push_back(v1[2]);  // z
    vertices_.push_back(v2[0]);
    vertices_.push_back(v2[1]);
    vertices_.push_back(v2[2]);
    vertices_.push_back(v3[0]);
    vertices_.push_back(v3[1]);
    vertices_.push_back(v3[2]);
}
...
{
    normals_.push_back(n1[0]);  // nx
    normals_.push_back(n1[1]);  // ny
    normals_.push_back(n1[2]);  // nz
    normals_.push_back(n2[0]);
    normals_.push_back(n2[1]);
    normals_.push_back(n2[2]);
    normals_.push_back(n3[0]);
    normals_.push_back(n3[1]);
    normals_.push_back(n3[2]);
}
...
{
    texCoords_.push_back(t1[0]); // s
    texCoords_.push_back(t1[1]); // t
    texCoords_.push_back(t2[0]);
    texCoords_.push_back(t2[1]);
    texCoords_.push_back(t3[0]);
    texCoords_.push_back(t3[1]);
}
...
void Icosphere::addSubLineIndices(unsigned int i1,
                                  ...
                                  unsigned int i6)
{
    lineIndices_.push_back(i1);      // i1 - i2
    lineIndices_.push_back(i2);
    lineIndices_.push_back(i2);      // i2 - i6
    lineIndices_.push_back(i6);
    lineIndices_.push_back(i2);      // i2 - i3
    lineIndices_.push_back(i3);
    lineIndices_.push_back(i2);      // i2 - i4
    lineIndices_.push_back(i4);
    lineIndices_.push_back(i6);      // i6 - i4
    lineIndices_.push_back(i4);
    lineIndices_.push_back(i3);      // i3 - i4
    lineIndices_.push_back(i4);
    lineIndices_.push_back(i4);      // i4 - i5
    lineIndices_.push_back(i5);
}
...
unsigned int Icosphere::addSubVertexAttribs(const float v[3], const float n[3], const float t[2])
{
    ...
    if(Icosphere::isSharedTexCoord(t))
    {
        ...
        const std::pair<float, float> key = std::make_pair(t[0], t[1]);
        const auto iter = sharedIndices_.find(key);
        if(iter == sharedIndices_.end())
        {
            addVertex(v[0], v[1], v[2]);
            addNormal(n[0], n[1], n[2]);
            addTexCoord(t[0], t[1]);
            index = static_cast<unsigned>(texCoords_.size() / 2 - 1);
            sharedIndices_[key] = index;
        }
        else
        {
            index = iter->second;
...
void Icosphere::computeFaceNormal(const float v1[3], const float v2[3], const float v3[3], float n[3])
{
    ...
    const float ex1 = v2[0] - v1[0];
    const float ey1 = v2[1] - v1[1];
    const float ez1 = v2[2] - v1[2];
    const float ex2 = v3[0] - v1[0];
    const float ey2 = v3[1] - v1[1];
    const float ez2 = v3[2] - v1[2];
...
void Icosphere::computeHalfVertex(const float v1[3], const float v2[3], float length, float newV[3])
{
    ...
    newV[1] = v1[1] + v2[1];
    newV[2] = v1[2] + v2[2];
    const float scale = computeScaleForLength(newV, length);
    newV[0] *= scale;
    newV[1] *= scale;
    newV[2] *= scale;
}
...
bool Icosphere::isSharedTexCoord(const float t[2])
{
    ...
    const int count = static_cast<int>(sizeof(segments) / sizeof(segments[0]));
